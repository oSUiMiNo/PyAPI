...
public class PyCreator : Editor
{
    ...
    static string Dir => "Assets/StreamingAssets/Py"; // AssetDatabase用のパス
    ...
    public static void CreateNewPythonScript()
    {
        ...
        string py = Path.Combine(Dir, fileName); // AssetDatabase用のパス

        // ファイルが既に存在する場合は連番を付与
        int count = 1;
        while (File.Exists(py))
        {
            fileName = $"NewPythonScript_{count}.py";
            py = Path.Combine(Dir, fileName); // AssetDatabase用のパス
            count++;
        }

        // 空のファイルを作成
        File.Create(py).Close();

        // AssetDatabaseをリフレッシュ
        AssetDatabase.Refresh();

        // 作成したファイルを選択
        Object obj = AssetDatabase.LoadAssetAtPath(py, typeof(Object));
        ...
        using (StreamWriter writer = new StreamWriter(py))
        {
